...
def doMining(begin, end, depth, equal):
    #printList()
    results = []
    count = end - begin
    if count < minsup:
        ...
        results.append([list(notes[begin:begin+depth]), count])
    ...
    #print "idx: ", idx
    range = b-a
    results += doMining(begin, begin+range, depth, False)
    # "000" is the end of the music
    if t != "000": 
	#print begin+range, range+a+end-d-1, depth+1
	results += doMining(begin+range, range+a+end-d-1, depth+1, True)
    range = d - c
    results += doMining(end-range, end, depth, False)
    return results

def mining(notes, minsup):
    for i in xrange(len(notes)-1):
        idx.append(i)
    return doMining(0, len(notes)-1, 0, False)


n = Notes("mozart")
notes = n.generateNotes("mozart-k331-1.txt")
minsup = 2
for note in mining(notes, minsup):
    print note[0], note[1]
